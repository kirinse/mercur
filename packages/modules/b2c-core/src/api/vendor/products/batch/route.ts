...
import { VendorBatchUpdateProductsType } from '../validators';
...
export const POST = async (
  ...
  const productIdsToChange = [
    ...req.validatedBody.update.map((p) => p.id),
  ...
  const unauthorizedProducts = productIdsToChange.filter(
    (id) => !ownedProductIds.has(id)
  );
  ...
  const isStatusChangeForbidden = req.validatedBody.update.some(
    (p) =>
      p.status && !['proposed', 'draft'].includes(p.status) && configurationRule
  );
