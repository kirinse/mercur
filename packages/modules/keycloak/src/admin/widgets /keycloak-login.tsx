...
import KeycloakIcon from '../components/keycloak-icon';
import { sdk } from '../lib/sdk';

const KEYCLOAK_AUTH_PROVIDER = 'keycloak';
...
const LoginWithKeycloak = () => {
  ...
  const keycloakLogin = async () => {
    const result = await sdk.auth.login('user', KEYCLOAK_AUTH_PROVIDER, {});
  ...
  const validateCallback = async () => {
    ...
    if (!userExists) {
      // Create user
      await sdk.client.fetch('/keycloak/users', {
        method: 'POST',
        body: {
